"""
Hybrid ALNS + DQN solver for VRPTW.

This is the original DQN-ALNS implementation, split into its own file.
"""
...
def solve_dqn_alns_vrptw(depot, customers, capacity, num_vehicles, model_path, iterations=300):
    """Solve VRPTW using DQN-ALNS hybrid."""
    start_time = time.time()

    print(f"[DQN-ALNS] Starting with {len(customers)} customers, {num_vehicles} vehicles")

    destroy_operators = [random_removal, route_removal]
    agent = DQNAgent(state_size=2, action_size=2)
    agent.load_weights(model_path)
    print(f"[DQN-ALNS] Loaded model: {model_path.split('/')[-1]}")
    ...
    print(
        f"[DQN-ALNS] Initial: cost={best_solution.total_cost:.1f}, "
    ...
    print(f"[DQN-ALNS] Completed in {execution_time:.1f}s")
    print(
        f"[DQN-ALNS] Final: cost={best_solution.total_cost:.1f}, "
        f"vehicles={len(best_solution.routes)}/{num_vehicles}"
    )
...
solve_dqn_vrptw = solve_dqn_alns_vrptw

__all__ = ["solve_dqn_alns_vrptw", "solve_dqn_vrptw"]
